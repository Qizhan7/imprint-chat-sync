...
async function syncAll() {
  const original = await chrome.storage.local.get(["syncDaysBack", "maxConversationsPerSync"]);
  // Remove both caps so full-history actually means full-history
  await chrome.storage.local.set({ syncDaysBack: 36500, maxConversationsPerSync: 99999 });
  ...
  try {
    ...
    await chrome.storage.local.set({
      ...
      maxConversationsPerSync: original.maxConversationsPerSync || 20,
    });
  }
}
